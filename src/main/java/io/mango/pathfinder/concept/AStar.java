...
/*
This is the one that you use in the service, do not fuck this up
 */
@Service
public class AStar {
    ...
    public AStar(int width, int height, int si, int sj, int ei, int ej, int[][]blocks) {
        ...
        openCells = new PriorityQueue<Cell>((Cell c1, Cell c2) -> Integer.compare(c1.finalCost, c2.finalCost));
        startCell(si, sj);
        endCell(ei, ej);
    ...
    public void process() {
        ...
        while(true) {
            ...
            if ( current.i - 1 >= 0) {
                t = grid[current.i - 1][current.j]; //[-1;0]
                updateCostIfNeeded(current, t, current.finalCost + V_H_COST);

                if (current.j - 1 >= 0) {
                    t = grid[current.i - 1][current.j - 1]; //[-1;-1]
            ...
            if (current.j - 1 >= 0) {
                t = grid[current.i][current.j - 1]; //[0;-1]
                updateCostIfNeeded(current, t, current.finalCost + V_H_COST);
            }
